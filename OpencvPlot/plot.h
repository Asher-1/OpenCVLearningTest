...
CPlot::CPlot()
{
    ...
    memset(Figure->imageData, 255, static_cast<size_t>(sizeof(unsigned char) * static_cast<size_t>(Figure->widthStep) * static_cast<size_t>(Figure->height)));
    //color
...
void CPlot::plot(T *X, T *Y, size_t Cnt, CvScalar color, char type,bool is_need_lined)
{
    ...
    for(size_t i = 0; i < Cnt;i++)
    {
        tempX = X[i];
        tempY = Y[i];
        data.push_back( cvPoint2D64f(static_cast<double>(tempX), static_cast<double>(tempY)));
    }
...
//    int chh = 10;
    ...
    int x = static_cast<int>((this->window_width - 2 * this->border_size ) / 2 + this->border_size - ( title_name.size() / 2.0 ) * chw);
...
void CPlot::xlabel(string xlabel_name, CvScalar label_color = Scalar(0,0,0))
{
    ...
    int x = w - bs - chw * static_cast<int>(xlabel_name.size());
    int y = static_cast<int>(x_axis_pos + bs / 1.5);
...
void CPlot::DrawAxis (IplImage *image)
{
    ...
    for (int i = 0; i < ceil( (x_axis_pos - bs) / y_scale_pixes ) + 1; i++)
    {
        ...
        cvPutText(image, text, cvPoint(static_cast<int>(bs / 5), static_cast<int>(x_axis_pos - i * y_scale_pixes)), &font, this->axis_color);
    }
    //Y负半轴
    ...
    cvPutText(image, text, cvPoint(static_cast<int>(w - bs/2 - static_cast<int>(strlen(text)) * chw), x_axis_pos), &font, this->text_color);
    ...
    for (int i = 1; i < ceil( gw / x_scale_pixes ) + 1; i++)
    {
        ...
        cvPutText(image, text, cvPoint(static_cast<int>(bs + i * x_scale_pixes - bs / 4), x_axis_pos + chh), &font, this->axis_color);
    }
}
...
void CPlot::DrawData (IplImage *image)
{
    ...
    //清屏
    memset(image->imageData, 255, static_cast<size_t>(sizeof(unsigned char) * static_cast<size_t>(Figure->widthStep) * static_cast<size_t>(Figure->height)));
    this->DrawAxis(image);
    ...
    int radius = 3;
    int slope_radius = static_cast<int>(( radius * 1.414 / 2 + 0.5));
    for(size_t i = 0; i < this->dataset.size(); i++)
    {
        for(size_t j = 0; j < this->dataset[i].size(); j++)
        {
            tempX = static_cast<int>((this->dataset[i][j].x - this->x_min) * this->x_scale);
            tempY = static_cast<int>((this->dataset[i][j].y - this->y_min) * this->y_scale);
            current_point = cvPoint(static_cast<int>(bs + tempX), static_cast<int>(image->height - (tempY + bs)));
...
public:
    ...
    Mat figure()
    {
        return cvarrToMat(this->Figure, true);
    }
};
